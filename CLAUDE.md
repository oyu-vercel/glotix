...
- Stories source of truth: `.txt` files under `docs/stories/<folder>/` (one folder per story), with optional `<slug>.extras.csv` sidecars next to each. The Angular app reads `web/public/assets/stories-index.json` and `web/public/assets/stories/<slug>.json`, generated by `web/scripts/build-stories.mjs` (walks subfolders recursively). `build:vocab` chains both scripts.
...
- [docs/stories-viewer.md](docs/stories-viewer.md) — stories feature reference (parser, JSON shape, extras CSV format).
