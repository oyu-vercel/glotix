...
- Stories source of truth: `.txt` files at `docs/stories/`, with optional `<slug>.extras.csv` sidecars for non-A2 vocabulary. The Angular app reads `web/public/assets/stories-index.json` and `web/public/assets/stories/<slug>.json`, generated by `web/scripts/build-stories.mjs`. `build:vocab` chains both scripts.
