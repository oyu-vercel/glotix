...
async function loadExtras(slug) {
  const path = resolve(storiesDir, `${slug}.extras.csv`);
...
async function buildStory(slug, rawText, headwordMap, indirectMap) {
  ...
  const extras = await loadExtras(slug);
...
async function main() {
  ...
  try {
    ...
    console.log(`No stories directory at ${storiesDir} — nothing to build.`);
  ...
  const txtFiles = entries.filter((f) => extname(f).toLowerCase() === '.txt').sort();
  ...
  for (const txt of txtFiles) {
    ...
    const raw = await readFile(resolve(storiesDir, txt), 'utf8');
    const story = await buildStory(slug, raw, headwordMap, indirectMap);
